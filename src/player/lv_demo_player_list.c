...
LV_IMAGE_DECLARE(img_lv_demo_player_btn_list_play);
LV_IMAGE_DECLARE(img_lv_demo_player_btn_list_pause);
...
lv_obj_t *lv_demo_player_list_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(list, LV_HOR_RES, LV_VER_RES - LV_DEMO_PLAYER_HANDLE_SIZE);
    lv_obj_set_y(list, LV_DEMO_PLAYER_HANDLE_SIZE);
    lv_obj_add_style(list, &style_scrollbar, LV_PART_SCROLLBAR);
    lv_obj_set_flex_flow(list, LV_FLEX_FLOW_COLUMN);

    uint32_t track_id;
    for (track_id = 0; lv_demo_player_get_title(track_id); track_id++)
    {
    ...
    lv_demo_player_list_button_check(0, true);
...
void lv_demo_player_list_button_check(uint32_t track_id, bool state)
{
    ...
    if (state)
    {
        ...
        lv_image_set_src(icon, &img_lv_demo_player_btn_list_pause);
        lv_obj_scroll_to_view(btn, LV_ANIM_ON);
    }
    else
    {
        lv_obj_remove_state(btn, LV_STATE_CHECKED);
        lv_image_set_src(icon, &img_lv_demo_player_btn_list_play);
    }
}
...
static lv_obj_t *add_list_button(lv_obj_t *parent, uint32_t track_id)
{
    uint32_t t = lv_demo_player_get_track_length(track_id);
    char time[32];
    lv_snprintf(time, sizeof(time), "%" LV_PRIu32 ":%02" LV_PRIu32, t / 60, t % 60);
    const char *title = lv_demo_player_get_title(track_id);
    const char *artist = lv_demo_player_get_artist(track_id);
    ...
    LV_IMAGE_DECLARE(img_lv_demo_player_list_border);
    lv_obj_t *border = lv_image_create(btn);
    lv_image_set_src(border, &img_lv_demo_player_list_border);
