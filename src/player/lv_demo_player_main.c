...
static void create_media_thumbnail(lv_obj_t *parent, media_file_t *media, int index)
{
    ...
    if (media->is_video) {
        ...
        if (media->is_gif) {
            // For GIF files, show a placeholder since GIF is not supported
            lv_obj_t *placeholder = lv_label_create(thumb_cont);
            lv_label_set_text(placeholder, "GIF\n文件");
            lv_obj_set_style_text_color(placeholder, lv_color_white(), 0);
            lv_obj_set_style_text_font(placeholder, &lv_font_source_han_sans_sc_16_cjk, 0);
            lv_obj_set_style_text_align(placeholder, LV_TEXT_ALIGN_CENTER, 0);
            lv_obj_center(placeholder);
        } else {
            // For supported images, load the actual image with scaling
            ...
            // Set to maintain aspect ratio and fit within container
